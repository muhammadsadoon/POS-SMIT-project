...
const ProjectPOS = () => {
  ...
  const fetchProducts = async () => {
    if (!projectId) return;
    const { data } = await supabase.from("products").select("*").eq("project_id", projectId).gt("stock", 0).order("name");
    setProducts(data || []);
  };
  ...
  const checkout = async (paymentMethod: string) => {
    ...
    try {
      const { data: sale, error: saleError } = await supabase
        .from("sales")
        .insert({ total, payment_method: paymentMethod, user_id: user.id, project_id: projectId })
        .select()
        .single();
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 h-[calc(100vh-7rem)]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="mb-4">
          <h1 className="text-2xl lg:text-3xl font-bold tracking-tight">Point of Sale</h1>
          <div className="flex items-center gap-2 mt-3">
            <div className="relative flex-1 max-w-sm">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <Input placeholder="Search products or barcode..." className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
              ...
              <CardContent className="p-4">
                ...
                {p.barcode && (
                  <div className="mt-2 pt-2 border-t border-border/50 flex justify-center">
                    <Barcode value={p.barcode} width={1} height={30} fontSize={10} margin={0} displayValue={true} />
                  </div>
                )}
              </CardContent>
            </Card>
      ...
      {/* Cart */}
      <Card className="w-full lg:w-80 xl:w-96 border-0 shadow-lg flex flex-col shrink-0 max-h-[50vh] lg:max-h-none">
